...
from WikiledNlp.learning.BasicLearning import LinerClassifier, RbfClassifier
from WikiledNlp.utilities.Lexicon import LexiconHelper


class LexiconDiscover(object):

    def __init__(self, vector_manager, domain_seeds, include_hash_tags=False):
        ...
        if include_hash_tags:
            logger.info('Including #hashtags: %i', len(self.vector.hash_tags))
            for item in self.vector.hash_tags:
                self.lexicon[item] = 0
    ...
    def construct(self, result):
        ...
        if self.balanced is True and self.take is None:
