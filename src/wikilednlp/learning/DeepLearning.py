...
from tensorflow_core.python.keras.backend import set_image_data_format
...
class BaseDeepStrategy(ABC):
    ...
    def get_embeddings(self):
        ...
        if not self.trainable_embeddings:
            ...
            return layers.Embedding(vectors.shape[0], vectors.shape[1])
        ...
        return layers.Embedding(vectors.shape[0], vectors.shape[1],
                                embeddings_initializer=tf.keras.initializers.Constant(vectors), trainable=False)

    def add_output(self, model):
        if self.output_drop_out is not None:
            model = layers.Dropout(self.output_drop_out)(model)
        output = layers.Dense(self.total_classes, activation='softmax', name='Output')(model)
        return output
    ...
    def test(self, test_x, test_y):
        ...
        test_x = keras.preprocessing.sequence.pad_sequences(test_x, value=Constants.PAD_ID, padding='post',
...
class CnnSentiment(BaseDeepStrategy):
    ...
    def construct_model(self):
        ...
        sequence_input = layers.Input(shape=(self.max_length,))
...
class LSTMSentiment(CnnSentiment):
    ...
    def construct_model(self):
        sequence_input = layers.Input(shape=(self.max_length,))
        embedded_sequences = self.get_embeddings()(sequence_input)
        x = layers.LSTM(self.lstm_size, return_sequences=False)(embedded_sequences)
        preds = self.add_output(x)
        model = keras.Model(sequence_input, preds)
        return model
...
class LSTMSentimentWithExternal(CnnSentiment):
    ...
    def construct_model(self):
        ...
        external_inputs = layers.Input(shape=(self.external_size,))
        external_inputs = layers.Dense(512, activation='relu')(external_inputs)
        external_inputs = layers.Dropout(0.5)(external_inputs)
        ...
        x = layers.Dense(64, activation='relu')(x)
        preds = self.add_output(x)
        model = keras.Model(inputs=[sequence_input, external_inputs], outputs=preds)
        return model
...
class BidiLTSMSentiment(LSTMSentiment):
    ...
    def construct_model(self):
        sequence_input = layers.Input(shape=(self.max_length,))
        embedded_sequences = self.get_embeddings()(sequence_input)
        x = layers.Bidirectional(layers.LSTM(self.lstm_size))(embedded_sequences)
        preds = self.add_output(x)
        model = keras.Model(sequence_input, preds)
        return model
